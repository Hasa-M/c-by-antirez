...
/* Translate the specified x,y grid point into the index in the linear
 * array. This function implements wrapping, so both negative and
 * positive coordinates that are out of the grid will wrap around */
...
/* Show the grid on the screen, clearing the teminal using the required
...
/* Set all the grid cells to the specificed state */
    ...
    for (int xOffset = -1; xOffset <= 1; xOffset++)
    {
        for (int yOffset = -1; yOffset <= 1; yOffset++)
        {
            if (xOffset == 0 && yOffset == 0)
                continue;
            if (get_cell(grid, x + xOffset, y + yOffset) == ALIVE)
                alive++;
        }
    }
    return alive;
...
void new_state(char *old, char *new)
{
    for (int y = 0; y < GRID_ROWS; y++)
    {
        for (int x = 0; x < GRID_COLS; x++)
        {
            ...
            if (n_alive == 3 || (n_alive == 2 && get_cell(old, x, y) == ALIVE))
            {
                set_cell(new, x, y, ALIVE);
            }
            else
            {
                set_cell(new, x, y, DEAD);
            }
        }
    }
}
...
sulla base del transcript, indicare le cose che non si possono utilizzare anche se si sarebbe dovuto come implementazione "ottimale" (struct per le define).

COntinue viene dichiarato per la prima volta nel corso in questa lezione di esercizio.

Sanfilippo worning about this: Warning: this program is odd C code because it's an example using just a subset of things learned by my students so far in my YouTube channel.